  ...
  return (
    ...
    >
      <div
        ...
        onClick={stop}
      >
        <div className="grid grid-cols-1 lg:grid-cols-5 min-h-[500px] sm:min-h-[600px]">
          {/* Visual side */}
          <div className="relative hidden lg:block lg:col-span-2">
            <img
              src={appData.images.loginModal}
              alt="Delicious pizza and food"
              className="h-full w-full object-cover"
              loading="lazy"
            />
            <div className="absolute inset-0 bg-gradient-to-br from-orange-600/20 via-transparent to-black/30" />
            <div className="absolute inset-0 flex flex-col justify-between p-8">
              ...
              <div className="space-y-4">
                <div className="rounded-2xl bg-white/90 p-4 backdrop-blur-sm">
                  <h4 className="font-semibold text-gray-900">{appData.modals.login.welcomeBonus.title}</h4>
                  <p className="text-sm text-gray-600 mt-1">{appData.modals.login.welcomeBonus.description}</p>
                </div>
                <div className="flex gap-2">
                  <div className="rounded-xl bg-white/20 px-3 py-2 text-xs text-white backdrop-blur-sm">{appData.features.delivery.icon} {appData.features.delivery.time}</div>
                  <div className="rounded-xl bg-white/20 px-3 py-2 text-xs text-white backdrop-blur-sm">{appData.features.security.icon} {appData.features.security.text}</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-3 flex flex-col">
            <div className="flex-1 p-4 sm:p-8 lg:p-12">
              <div className="flex items-start justify-between mb-6 sm:mb-8">
                <div>
                  <h3 className="text-xl sm:text-2xl font-bold text-gray-900">{appData.modals.login.title}</h3>
                  <p className="mt-2 text-sm sm:text-base text-gray-600">{appData.modals.login.subtitle}</p>
                </div>
                <button
                  onClick={onClose}
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="flex items-center justify-between">
                  <label className="inline-flex items-center gap-3 text-sm text-gray-600 cursor-pointer">
                    <input type="checkbox" className="rounded border-gray-300 text-orange-600 focus:ring-orange-500 focus:ring-2" />
                    <span>Keep me signed in</span>
                  </label>
                  <button type="button" className="text-sm font-medium text-orange-600 hover:text-orange-500 transition-colors">
                    Forgot password?
                  </button>
                </div>
            ...
            <div className="border-t border-gray-100 p-6 bg-gray-50/50">
              <p className="text-center text-sm text-gray-600">
                ...
                <button onClick={onOpenSignup} className="font-semibold text-orange-600 hover:text-orange-500 transition-colors ml-1">
                  Sign up for free
                </button>
              </p>
            </div>
